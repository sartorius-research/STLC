...
class LumpedKineticModel:
    ...
    def __post_init__(self) -> None:
        ...
        for i in range(self.nc):
            ...
            self.ka[i] = self.parameters[i].ka
            self.kd[i] = self.parameters[i].kd
            self.qm[i] = self.parameters[i].qm
...
def _calc_qstar(model: LumpedKineticModel, yt: np.ndarray, i: int) -> np.ndarray:
    ...
    for j in range(model.nc):
        qj = (
            np.diag(yt[model.q_end * j + model.q_idx : model.q_end * j + model.q_end])
            / model.qm[j]
        )
        total += qj
    calc_qstar = model.ka[i] * model.qm[i] * np.diag((1 - np.diag(total)))
    return calc_qstar
...
def _assemble_complete_matrix(model: LumpedKineticModel, yt: np.ndarray) -> np.ndarray:
    ...
    for i in range(0, model.nc):
        ...
        Qcstar = -((1 - model.ep[i]) / model.ep[i]) * qstar

        Qcstar[0, :] = 0
        Qcstar[-1, :] = 0
        Qc = ((1 - model.ep[i]) / model.ep[i]) * model.kd[i] * np.eye(model.nz2)
        Qc[0, :] = 0
        Qc[-1, :] = 0
        ...
        Qqstar = qstar
        Qq = -model.kd[i] * np.eye(model.nz2)
        C[
            model.q_end * i + model.q_idx : model.q_end * i + model.q_end,
            model.q_end * i + model.c_idx : model.q_end * i + model.c_end,
        ] = Qqstar
